 ...
 * Tests the actual microgpt.c implementation functions
 * Compile: gcc -O3 -o test_microgpt test_microgpt.c -lm
 */
...
// Include the actual implementation (as extern functions)
// We'll define the functions as extern and link with microgpt.o

// Configuration
#define EPSILON 1e-9
#define N 624

// Forward declarations from microgpt.c
#define SEED 42
#define N_EMBD 16
#define N_HEAD 4
#define N_LAYER 1
...
extern void init_state_dict(StateDict *sd, MersenneTwister *rng);
...
void test_prng_random_matches_python() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (!approx_eq(actual, PRNG_RANDOM_100[i], EPSILON)) {
            ...
            passed = 0;
            if (i >= 20) break; // Stop after first 20 failures
        }
    }
...
void test_value_add() {
    ...
    // Need to access value data - this requires exposing the value pool
    // For now, just test that it doesn't crash
    printf("  PASS: add executed without error\n");
}
...
void test_value_mul() {
    ...
    printf("  PASS: mul executed without error\n");
}
