...
export function createStateDict(
  ...
  for (let i = 0; i < config.nLayer; i++) {
    ...
    stateDict[`layer${i}.mlp_fc1`] = matrix(
      rng,
      4 * config.nEmbd,
      config.nEmbd
    );
    stateDict[`layer${i}.mlp_fc2`] = matrix(
      rng,
      config.nEmbd,
      4 * config.nEmbd,
      0
...
export async function main(): Promise<void> {
  ...
  for (let step = 0; step < numSteps; step++) {
    ...
    console.log(
      `step ${String(step + 1).padStart(4)} / ${String(numSteps).padStart(
        4
      )} | loss ${loss.data.toFixed(4)}`
    );
  }
...
// Entry point
const isMainModule =
  process.argv[1]?.endsWith("microgpt.ts") ||
